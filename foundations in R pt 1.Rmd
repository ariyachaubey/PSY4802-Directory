...
```{r}
AnimalCookies <- cbind.data.frame(Animals, Cookies) #vectors are columns
AnimalCookies
```
...
AnimalCookies[,1] #column is 2nd number in []
AnimalCookies[,2] #gives you all observations
AnimalCookies[3, "Animals"]
AnimalCookies[3,1] #same as [3, "Animals"]
AnimalCookies[3, "Cookies"]
...
```{r}
AnimalCookies$Animals
AnimalCookies$Cookies
```
...
```{r}
tvshows<- c("Avatar", "Jujutsu Kaisen", "Lost", "Supernatural")
tvratings<- c(10,11,4,3)
```

3.  Make these vectors into a data frame.

```{r}
tvdata<- cbind.data.frame(tvshows, tvratings)
tvdata
```
